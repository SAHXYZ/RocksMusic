...
class Call(PyTgCalls):
    ...
    async def speedup_stream(self, chat_id: int, file_path, speed, playing):
        ...
        if str(speed) != "1.0":
            ...
            if not os.path.isfile(out):
                vs = {"0.5": 2.0, "0.75": 1.35, "1.5": 0.68, "2.0": 0.5}.get(
                    str(speed), 1.0
                )
    ...
    async def change_stream(self, client, chat_id):
        ...
        # SAFETY PATCH: queued can be None -> avoid "argument of type 'NoneType' is not iterable"
        if queued and isinstance(queued, str) and "live_" in queued:
            ...
            try:
                ...
                try:
                    file_path = _download_audio_strict(
            ...
            stream = (
                ...
                if video
                    ...
                    file_path, audio_parameters=HighQualityAudio()
                )
            )
            try:
                await client.change_stream(chat_id, stream)
